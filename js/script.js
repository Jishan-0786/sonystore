 ...
 * 2. If authenticated: renders PROFILE button/link pointing to profile.html.
...
async function updateNavbarAuthUI(sessionParam) {
    ...
    navs.forEach(nav => {
        let authBtn = nav.querySelector('#nav-auth-btn') || nav.querySelector('.nav-link-auth');
        ...
        if (!authBtn) {
            ...
            authBtn.id = 'nav-auth-btn';
            authBtn.classList.add('nav-link-auth');
        }
        ...
        if (user && (user.email || user.id)) {
            ...
            authBtn.title = `Logged in as ${userName}`;
